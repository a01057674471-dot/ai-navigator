...
제품 형태 변경, 라벨 재작성, 가짜 인증 마크, 과장된 크기, 내용물 추가, 새로운 효능 문구, 워터마크, 사람 손, 관련 없는 장식을 넣지 마세요.`,
  ...
  function openPromptRecipe(id) {
    ...
    $('#modal-body').innerHTML = `<div class="recipe-modal">
      ...
      <figure class="recipe-result-preview"><a href="/assets/prompt-previews/${escapeHtml(item.id)}.webp" target="_blank" rel="noreferrer"><img src="/assets/prompt-previews/${escapeHtml(item.id)}.webp" alt="${escapeHtml(item.title)} 프롬프트 AI 생성 결과 예시" onerror="this.hidden=true;this.parentElement.nextElementSibling.hidden=false" /></a><span class="recipe-preview-fallback" hidden><b>${escapeHtml(item.icon)}</b><small>이 레시피의 전용 결과 예시는 다음 단계에서 추가됩니다.</small></span><figcaption><strong>AI 생성 결과 예시</strong><span>이 레시피로 기대할 수 있는 분위기를 보여주는 샘플이에요. 실제 결과는 원본 사진과 사용하는 AI 모델에 따라 달라질 수 있어요.</span></figcaption></figure>
