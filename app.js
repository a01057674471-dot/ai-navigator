...
제목 후보 3개와 300자 이내 본문을 작성하세요. 핵심 요청과 마감일이 한눈에 보이게 하고 과도한 수식어는 줄이세요.` }
  ...
  function toolCard(tool, index, recommendation = false) {
    ...
    return `<article class="recommend-card" data-tool-id="${escapeHtml(tool.id)}">
      ...
      <div class="tool-head">${toolLogoMarkup(tool)}<div>${toolNameMarkup(tool)}<div class="tool-maker">${escapeHtml(tool.maker)}${deepToolProfiles[String(tool.id)] ? '<span class="deep-card-badge">심층 정보</span>' : ''}</div></div></div>
