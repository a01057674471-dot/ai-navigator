...
먼저 넓은 질문으로 시장의 용어와 주요 회사를 찾습니다. 다음으로 기간·지역·고객군을 좁혀 검색하고, 핵심 주장마다 원문 링크와 발표일을 기록합니다. 마지막에는 확인된 사실, 추정, 아직 모르는 내용을 나눠 표로 정리하면 의사결정에 바로 사용할 수 있습니다.`
  ...
  function mapNews(row) {
    return { id: row.id, label: row.label, date: formatDate(row.published_at || row.created_at || row.date), title: row.title, summary: row.summary, body: row.body || '', insight: row.insight || '', category: row.category || 'insight', source: row.source || '출처 미정', sourceUrl: row.source_url || '#' };
  }
...
제목 후보 3개와 300자 이내 본문을 작성하세요. 핵심 요청과 마감일이 한눈에 보이게 하고 과도한 수식어는 줄이세요.` }
  ...
  function isFreeEligible(tool) {
    return tool.priceType === 'free' || tool.priceType === 'freemium';
  }
