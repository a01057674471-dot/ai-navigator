...
제품 형태 변경, 라벨 재작성, 가짜 인증 마크, 과장된 크기, 내용물 추가, 새로운 효능 문구, 워터마크, 사람 손, 관련 없는 장식을 넣지 마세요.`,
  ...
  function openPromptRecipe(id) {
    ...
    $('#modal-body').innerHTML = `<div class="recipe-modal">
      ...
      <figure class="recipe-result-preview"><a href="/assets/prompt-previews/${escapeHtml(item.id)}.webp" target="_blank" rel="noreferrer"><img src="/assets/prompt-previews/${escapeHtml(item.id)}.webp" alt="${escapeHtml(item.title)} 프롬프트 AI 생성 결과 예시" onerror="this.hidden=true;this.parentElement.nextElementSibling.hidden=false" /></a><span class="recipe-preview-fallback" hidden><b>${escapeHtml(item.icon)}</b><small>전용 결과 예시 준비 중</small></span><figcaption><strong>AI 생성 결과 예시</strong><span>이 레시피로 기대할 수 있는 분위기를 보여주는 샘플이에요. 실제 결과는 원본 사진과 사용하는 AI 모델에 따라 달라질 수 있어요.</span></figcaption></figure>
  ...
  function newsItem(item, archive = false) {
    ...
    const hasOriginal = Boolean(item.sourceUrl && item.sourceUrl !== '#');
    const contentType = hasOriginal ? '외부 기사 요약' : '편집팀 작성';
    return `<a href="${escapeHtml(href)}" class="news-item ${archive ? 'news-archive-card' : ''}" data-news-id="${escapeHtml(item.id)}" ${articlePath ? `data-news-path="${escapeHtml(articlePath)}"` : ''}>
      ...
      <div class="news-provenance"><span class="${hasOriginal ? 'external' : 'editorial'}">${contentType}</span><span>출처 · ${escapeHtml(item.source || '확인 중')}</span>${hasOriginal ? '<b>원문 확인 가능</b>' : ''}</div>
  ...
  function openNews(item) {
    ...
    const hasOriginal = Boolean(item.sourceUrl && item.sourceUrl !== '#');
    const contentType = hasOriginal ? '외부 기사 요약' : 'AI 네비게이터 편집글';
    $('#modal-body').innerHTML = `<article class="news-article"><div class="news-article-provenance"><span class="${hasOriginal ? 'external' : 'editorial'}">${contentType}</span><div><small>출처</small><strong>${escapeHtml(item.source || '확인 중')}</strong></div><div><small>발행일</small><strong>${escapeHtml(item.date || '날짜 미정')}</strong></div></div><p class="news-lead">${escapeHtml(item.summary)}</p>${body ? `<div class="news-article-body"><h4 class="news-section-title">전체 본문</h4>${renderNewsBody(body)}</div>` : '<p class="news-missing-body">이 기사의 전체 본문은 현재 작성 중입니다.</p>'}<aside class="news-insight-box"><h4>왜 중요한가</h4><p>${escapeHtml(item.insight)}</p></aside><div class="news-source-box"><h4>출처와 원문</h4><p>${escapeHtml(item.source || '출처 확인 중')}</p>${hasOriginal ? `<a class="modal-link news-original-link" href="${escapeHtml(item.sourceUrl)}" target="_blank" rel="noreferrer">외부 원문에서 직접 확인하기 ↗</a><small>링크를 열면 AI 네비게이터가 아닌 원문 제공 사이트로 이동합니다.</small>` : '<span class="news-editorial-note">이 글은 AI 네비게이터가 직접 작성한 편집 콘텐츠로 외부 원문이 없습니다.</span>'}</div>${!state.remoteNews ? '<p class="demo-note">현재 뉴스는 화면 검증을 위한 데모 콘텐츠입니다. Supabase를 연결하면 관리자 검수 후 발행된 뉴스가 표시됩니다.</p>' : ''}</article>`;
    $('#modal').classList.add('open');
  }
