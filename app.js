...
표지, 문제, 핵심 내용, 사례, 실행안, 결론 순서로 구성하고 시각자료 제안도 포함해 주세요.`
  ...
  function compareWinners(selected, key) {
    ...
    return selected.filter(tool => Number(tool[key]) === highest).map(tool => tool.name).join(' · ');
  }
  ...
  function renderComparePage() {
    ...
    $('#compare-page-cards').innerHTML = selected.map(tool => `<article class="compare-product-card">
      ...
      <h3>${escapeHtml(tool.name)}</h3><p>${escapeHtml(tool.maker)}</p>
    ...
    $('#compare-page-table').innerHTML = `<table class="compare-table dedicated">
      <tr><th>비교 항목</th>${cells(tool => `<td><div class="compare-name">${escapeHtml(tool.name)}</div></td>`)}</tr>
    ...
    $('#compare-verdicts').innerHTML = `
      <article><span>🏆 종합 추천</span><strong>${escapeHtml(overall.name)}</strong><p>품질·난이도·한국어·가격·보안을 고르게 고려했어요.</p></article>
